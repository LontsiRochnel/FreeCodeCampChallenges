...
function infected(days) {
    let numberInfected = 0;
    if (days >= 0) numberInfected = 1;
    for (let i = 1; i <= days; i++) {
        numberInfected *= 2;
        if (i % 3 == 0) {
            // Using ceil because we need to go the smallest integer greater than or equal to the numberInfected * 0.2
            numberInfected -= Math.ceil(numberInfected * 0.2);
        }
    }
